...
def compareSisterClades():
    ...
    for rep in range(1,101):
        for mask in [10,20,30,40,50]:
            ...
            with open('N1/missing_10_'+str(rep)+'_mask_'+str(mask)+'.n1.sister_clade') as f:
                for line in f:
                    if (line.strip()).endswith(':'):
                        myStrain = line.strip()[:-1]
            ...
            with open('N2/missing_10_'+str(rep)+'_mask_'+str(mask)+'.n2.sister_clade') as f:
                for line in f:
                    if (line.strip()).endswith(':'):
                        myStrain = line.strip()[:-1]
            ...
            with open('N3/missing_10_'+str(rep)+'_mask_'+str(mask)+'.n3.sister_clade') as f:
                for line in f:
                    if (line.strip()).endswith(':'):
                        myStrain = line.strip()[:-1]
            ...
            for l in lineageToSimTreeN1:
                ...
                myOutString += (str(rep)+'\t'+str(mask)+'\t'+l+'\t'+str(N1Idetical)+'\t'+str(NStat)+'\t'+str(notAtRoot))+'\n'
        print("Finished rep "+str(rep))
    open('compareSisNew1.txt','w').write(myOutString)
